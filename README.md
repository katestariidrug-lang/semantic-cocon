...
## TUI (Textual UI) — read-only наблюдатель
...
TUI **НЕ вычисляет и НЕ отображает FSM-состояние** принципиально.

TUI показывает:
- **сырые наблюдаемые факты** на диске (state/, outputs/)
- **текстовую проекцию контрактных условий README.md**
  без классификации lifecycle-состояний
  и без вывода S0…S6

TUI **не формирует**:
- FSM-state
- “текущее состояние lifecycle”
- derived-классификацию по приоритету состояний

Любая интерпретация lifecycle и определение FSM-состояния
остаются исключительной ответственностью enforcing CLI.
...
**Что TUI показывает:**
- наличие snapshot’ов и их идентификаторы
- факт approve (наличие `.approved`)
- факт MERGE (наличие merge-state)
- наличие результатов CORE / ANCHORS
- соответствие run_id ↔ snapshot_id ↔ merge_id по файловой структуре
- **наблюдаемое FSM-состояние**
- **Allowed actions / Forbidden actions** (с причиной как ссылкой на контрактное условие)

**Что TUI НЕ делает (запрещено, HARD):**
- не является enforcing-гейтом: не “разрешает/запрещает выполнение”, а только отображает read-only проекцию Allowed/Forbidden по контракту README.md
