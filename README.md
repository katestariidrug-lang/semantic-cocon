...
# merge_id присутствует в stdout merge_pass2 (в message или в JSON evidence), в формате контрактного вывода
...
Формат может быть текстовым или JSON, но семантика полей обязательна и стабильна. Для CLI-логов дополнительно применяется строковый формат: `[LEVEL] ERROR_CODE: message`.
...
Примечание: в строковом формате ниже `SUCCESS` соответствует `PASS`, а `VIOLATION` соответствует `BLOCKER`.

### Формат вывода (stdout / stderr)

Все сообщения CLI-гейтов (`orchestrator`, `merge_pass2`, `check_deliverables.py`) обязаны использовать единый,
машиночитаемый формат строк:
...
Где:
- LEVEL — одно из значений: PASS, FAIL, BLOCKER
- ERROR_CODE — канонический код ошибки (UPPER_SNAKE_CASE)
- message — человекочитаемое описание

Соответствие уровней и exit code:
- PASS → 0
- FAIL → 1
- BLOCKER → 2