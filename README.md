...
Назначение: предоставить **наблюдение фактов состояния** без влияния
на lifecycle, архитектуру или порядок шагов.

Общее правило (HARD):

- read-only инструменты **ничего не решают**
- **ничего не разрешают и не запрещают**
- **ничего не пишут** в `state/`, `outputs/`, `input/`
- **не запускают** subprocess / enforcing CLI
- **не интерпретируют** данные и не дают рекомендаций
...
**Роль (строго):**  
Textual TUI — это **read-only наблюдатель фактов**, не участник pipeline.

TUI **читает и отображает ТОЛЬКО факты**, уже зафиксированные на диске,
без вычислений, интерпретаций и выводов.
...
**Что TUI показывает (факты, не интерпретации):**
...
- не определяет «можно / нельзя продолжать»
