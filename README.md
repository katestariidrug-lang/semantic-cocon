...
В отдельных разделах README.md (включая governance-таблицу entrypoints)
контракт **исполняется напрямую кодом**.

Такие разделы считаются **ABI-контрактами** между документацией
и enforcement-инструментами и **не могут изменяться косметически**
без синхронного изменения кода enforcement.
...
#### Архитектурные решения человека (Design-time, HARD)

На этапе PASS_1 человек **ИМЕЕТ ПРАВО и ОБЯЗАН**:

- формулировать `input/task.json`;
- изменять архитектурные требования **до** запуска PASS_1;
- изменять системные промпты:
  - `prompts/pass_1_decide.md`,
  - `prompts/pass_2_execute_core.md`,
  - `prompts/pass_2_execute_anchors.md`
  **ТОЛЬКО до формирования snapshot**;
- интерпретировать результат PASS_1
  **исключительно как архитектурное предложение**, а не как исполнимое решение;
- принять решение о выпуске snapshot
  или о повторном запуске PASS_1 с иными входными условиями.

Запрещено на этом этапе:
- трактовать PASS_1 как выполнение или частичный результат;
- запускать PASS_2 без APPROVE;
- «подкручивать» snapshot вручную.

Все архитектурные решения человека
**фиксируются только через выпуск snapshot**.
...
⚠️ КРИТИЧЕСКОЕ УТОЧНЕНИЕ (HARD):

Smoke-test **НЕ**:
- доказывает отсутствие enforcing-зависимостей;
- подтверждает helper / read-only статус entrypoint’а;
- заменяет или ослабляет audit_entrypoints.

Даже полностью зелёный smoke-test **НЕ МОЖЕТ** компенсировать:
- отсутствие audit_entrypoints,
- или обнаруженную enforcing-зависимость.

Smoke-test — вспомогательный предохранитель, **НЕ governance-доказательство**.
...
GOVERNANCE_VIOLATION    # нарушение governance-инварианта (запреты README.md),
                        # включая:
                        # - enforcing-зависимость helper-инструментов
                        #   (включая случаи без write-side-effects),
                        # - запрещённые импорты enforcing-кода,
                        # - любые серые зоны между helper и enforcing
```
...
## Human Decision Surface (HARD)

**Human Decision Surface** — это **единственная контрактно допустимая поверхность
человеческих решений** в проекте `semantic-cocon`.

Human Decision Surface определяет:
- **ГДЕ** человек имеет право принимать решения,
- **В КАКОЙ ФОРМЕ** эти решения фиксируются,
- **КАКИМИ АРТЕФАКТАМИ** они подтверждаются,
- и **ЧТО СЧИТАЕТСЯ НАРУШЕНИЕМ**, даже если код “отработал”.

Любое человеческое действие,
которое не может быть однозначно выражено
через перечисленные ниже артефакты,
считается **out-of-scope** и архитектурно недействительным.

**Human Decision Surface** — это **единственная контрактно допустимая поверхность
человеческих решений** в проекте `semantic-cocon`.

Человек **имеет право принимать решения ТОЛЬКО в рамках этой поверхности**.
Любое решение человека вне описанных ниже границ
считается **нарушением архитектурного контракта**,
независимо от того, «сработал ли код».

Human Decision Surface существует для:
- явного отделения человеческих решений от enforcing-кода;
- запрета неявных или “помогающих” решений через UX;
- обеспечения воспроизводимости и audit-доказуемости pipeline.

### Классификация решений человека (HARD)

Человек **имеет право** принимать решения **ТОЛЬКО** в следующих формах,
каждая из которых привязана к конкретному артефакту:

#### 1. Архитектурные решения (Design-time, HARD)
Допустимы **ТОЛЬКО ДО PASS_1** и фиксируются через:
- редактирование `input/task.json`,
- редактирование `prompts/pass_1_decide.md`,
- редактирование `prompts/pass_2_execute_core.md`,
- редактирование `prompts/pass_2_execute_anchors.md`.

Любое изменение этих файлов:
- **ПОСЛЕ формирования snapshot**,
- **БЕЗ выпуска нового snapshot**,

считается **нарушением архитектурного контракта**,
независимо от результата выполнения pipeline.

#### 2. Подтверждающее решение (Approval-time, HARD)
Единственная допустимая runtime-форма человеческого решения — **APPROVE**.

Фиксируется **ТОЛЬКО** через:
- наличие файла  
  `state/approvals/<sha256>.approved`

`APPROVE`:
- не является валидацией,
- не является разрешением EXECUTE,
- не является технической проверкой,
- не может быть заменён UX / CLI / автоматикой.

#### 3. Запрещённые решения (Out-of-scope, HARD)
К запрещённым относятся **ВСЕ** формы решений, включая (неисчерпывающий список):

- интерпретация вывода UX/TUI как “можно / нельзя”;
- принятие решений на основе “Allowed / Forbidden” списков UX;
- ручное редактирование `state/` или `outputs/`;
- запуск lifecycle-команд “по ощущению”;
- игнорирование BLOCKER с аргументом “человек так решил”.

Любое такое действие трактуется как:
- нарушение Human Decision Surface,
- нарушение governance-инварианта,
- `[BLOCKER] GOVERNANCE_VIOLATION`.

Решения человека делятся на **три и только три класса**:

1. **Архитектурные решения (Design-time)**  
2. **Подтверждающие решения (Approval-time)**  
3. **Запрещённые решения (Out-of-scope)**  

Любое человеческое действие,
не попадающее в классы (1) или (2),
автоматически относится к классу (3).
...
#### Подтверждающее решение человека (Approval-time, HARD)

`APPROVE` — это **единственное допустимое runtime-решение человека** в lifecycle.

Через APPROVE человек:
- подтверждает **готовность принять последствия** архитектурного решения;
- принимает точку невозврата (snapshot становится immutable);
- **НЕ валидирует** snapshot;
- **НЕ интерпретирует** техническую корректность;
- **НЕ принимает решение о допустимости EXECUTE** (это делает enforcing CLI).

Отказ от APPROVE:
- не является ошибкой;
- не считается FAIL или BLOCKER;
- означает осознанный отказ от исполнения snapshot.

Повторное APPROVE для одного snapshot:
- допустимо,
- не меняет архитектурного смысла,
- не является новым решением.
...
#### Определение enforcing-зависимости (HARD, КАНОНИЧЕСКОЕ)

**Enforcing-зависимость** — это *любая* зависимость (прямая или косвенная)
helper / read-only инструмента от enforcing-кода проекта,
независимо от наличия write-side-effects.

Это определение является **единственным каноническим**
для всего README.md.

Все упоминания enforcing / helper границы,
включая UX, TUI, wizard’ы, smoke-тесты и audit-разделы,
**интерпретируются исключительно через данное определение**
и **НЕ имеют права** вводить собственные альтернативные критерии
(например "если не пишет — значит helper", "если только константы — ок", "если только read-only — ок").

##### Канонический критерий "что такое enforcing-код" (HARD)

**Enforcing-код проекта** — это любой модуль/файл/entrypoint,
который по README.md классифицирован как:
- `TYPE: enforcement`, **или**
- `Класс: enforcing` в governance-таблице entrypoints, **или**
- выполняет/делегирует lifecycle enforcement, drift guard, approve/execute/merge/post-check,
  либо возвращает контрактные `[PASS]/[FAIL]/[BLOCKER]` вердикты как часть enforcing-поведения.

Это критерий **по контрактной классификации README.md**, а не "по намерению автора кода".

К enforcing-зависимостям относятся (неисчерпывающий перечень):

- прямой импорт enforcing-модуля;
- импорт enforcing-модуля через alias;
- dynamic import (`importlib`, `__import__`, строковые имена модулей);
- транзитивный импорт через "безопасные утилиты", shared helpers или re-export;
- обращение к enforcing-коду для:
  - вычисления условий,
  - интерпретации lifecycle,
  - определения допустимости шага,
  - формирования verdict/классификации,
  - чтения/использования enforcing-констант, таблиц, предикатов и "общих утилит".

**Критическое правило:**
> Отсутствие записи в `state/` **НЕ** означает, что код является helper.

⚠️ ДОПОЛНЕНИЕ (HARD):
> Отсутствие записи и зелёный smoke-test **НЕ** означают,
> что enforcing-зависимостей нет.

Если инструмент использует enforcing-код для логики или выводов —
он считается **enforcing по контракту**, даже если физически ничего не пишет.
...
- **read-only** — entrypoint **не имеет права**:
  ...
  - иметь enforcing-зависимости (включая "только константы/утилиты").
...
Данный заголовок и **непосредственно следующая за ним таблица**
являются **ABI-контрактом** между:

- `README.md`,
- `scripts/audit_entrypoints.py`,
- CI / governance pipeline.

Требования ABI-контракта:

- Заголовок **ДОЛЖЕН** сохраняться без переименований.
- Таблица **ДОЛЖНА** располагаться сразу под этим заголовком.
- Структура таблицы и порядок колонок являются **частью ABI**.
- Любое структурное изменение заголовка или таблицы
  **ЛОМАЕТ ABI** и обязано приводить к `BLOCKER`
  в `audit_entrypoints`.

Нарушение ABI-контракта трактуется как
**LIFECYCLE_VIOLATION**.
...
| `python scripts/tui.py` | helper | read-only | нет | не требуется |
| `python scripts/ui.py` | helper | read-only | нет | не требуется |
...
Read-only статус entrypoint **считается допустимым по контракту**
**ТОЛЬКО если одновременно выполняются все условия**:

1. Entrypoint присутствует в governance-таблице entrypoints
   и помечен как `helper / read-only`.
2. `scripts.audit_entrypoints.py`:
   - не обнаруживает enforcing-entrypoints вне README.md,
   - не фиксирует расхождений inventory.
3. Для entrypoint существует **явное smoke-доказательство** отсутствия
   write-side-effects (snapshot дерева до/после выполнения).
4. Entrypoint **не импортирует enforcing-модули**
   (прямо, транзитивно или динамически).

Отсутствие любого пункта = **серой зоны не устранено**
и трактуется как **GOVERNANCE_VIOLATION**.

⚠️ ВАЖНО (HARD):

Наличие smoke-доказательства **НИКОГДА** не означает,
что enforcing-зависимостей нет.

Smoke подтверждает **только отсутствие write-side-effects**.
Audit подтверждает **отсутствие enforcing-зависимостей**.

Эти доказательства **не взаимозаменяемы**.

Если `audit_entrypoints` обнаруживает enforcing-зависимость
у entrypoint, помеченного как `helper / read-only`,
это является основанием для:
- `[BLOCKER] GOVERNANCE_VIOLATION`,
- отказа pipeline независимо от наличия write-side-effects.

**Важно:**
- декларации в README.md **не считаются доказательством** сами по себе;
- доказательством считается только комбинация: governance-инвентаря + audit + smoke-факта.

⚠️ ИЕРАРХИЯ ДОКАЗАТЕЛЬСТВ (HARD):

- `audit_entrypoints` — **authoritative и обязательный**;
- smoke-факты **вспомогательные** и **никогда не самостоятельные**.

Отсутствие или отрицательный результат audit_entrypoints
**НЕ МОЖЕТ** быть компенсирован smoke-фактами
независимо от их количества или "зелёности".
...
#### Универсальный запрет enforcing-импортов для helper-инструментов

Любой entrypoint, помеченный как `helper` / `read-only`, **СТРОГО ЗАПРЕЩЕНО**:

- импортировать enforcing-модули проекта **в любой форме**;
- ссылаться на enforcing-код по строковым именам;
- использовать enforcing-утилиты, даже "только для чтения";
- вызывать enforcing-логику для расчётов, условий или классификаций.

Это правило действует **для всех helper-инструментов**, а не только для TUI/UX.

**Нарушение этого правила считается:**
- governance-нарушением,
- архитектурным дефектом,
- основанием для `[BLOCKER] GOVERNANCE_VIOLATION`,
даже если:
- запись в `state/` отсутствует,
- lifecycle формально не изменён,
- результат выглядит "информационным".

Read-only статус определяется **не отсутствием записи**,  
а **отсутствием зависимости от enforcing-кода**.
...
Назначение: предоставить **наблюдение фактов состояния** и
**информационную проекцию lifecycle-контракта** без влияния
на pipeline (enforcement выполняется только enforcing CLI).

Общее правило (HARD):

- read-only инструменты **ничего не решают** и **не являются enforcing-гейтами**
- read-only инструменты могут показывать:
  - **наблюдаемое FSM-состояние** (как классификацию по фактам на диске)
  - **Contract-allowed commands (informational only) / Contract-forbidden commands (informational only)** как **отражение контракта README.md**
- при показе Contract-allowed / Contract-forbidden:
  - это **строго информационная классификация**, а не разрешение
  - это **не сигнал к действию**
  - это **не рекомендация**
  - это **не гарантия успеха**: enforcing CLI всё равно выполнит проверки и может вернуть FAIL/BLOCKER
  - причины запрещённости формулируются **только** как ссылки на контрактные правила/предикаты (approve missing, merge-state exists, fingerprints mismatch и т.п.), без "советов" и без "следующего шага"
- **ничего не пишут** в `state/`, `outputs/`, `input/`
- **не запускают** subprocess / enforcing CLI
- **не модифицируют** окружение и не имеют side-effects
...
## UX (TUI/GUI) — non-authoritative thin interface (HARD)

Дополнение (HARD):
- Допускается реализация UI (GUI/Web UI) как **helper / read-only** entrypoint, если он:
  - является **presentation-only**;
  - не инициирует и не запускает enforcing CLI (не является драйвером workflow);
  - не формирует “next step”, progress, verdict или рекомендации;
  - не имеет enforcing-зависимостей (по определению enforcing-зависимости в этом README).
- Любой UI entrypoint обязан быть явно добавлен в governance-таблицу entrypoints
  и классифицирован как `helper / read-only`, иначе он считается **нелегальным**.

UX-слой в semantic-cocon (любой TUI/GUI/Web UI) является **неавторитетной тонкой оболочкой** над контрактом README.md и enforcing CLI.

Важное разграничение (контракт):
- **Поведение/ограничения UX, описанные в этом разделе, являются частью архитектурного контракта README.md**.
- **Конкретная реализация UX (код UI) не является источником истины и не имеет контрактной силы**: её можно менять или удалять,
  пока она остаётся в рамках запретов и классификации entrypoints / write-matrix.

Ключевой принцип: UX можно удалить целиком, и **корректность pipeline не изменится**.
Корректность обеспечивается только enforcing CLI и canonical state в `state/`.

Доказательства non-authoritative/read-only статуса UX фиксируются governance-артефактами,
описанными в README.md (entrypoints inventory + read-only smoke-доказательства, например `python -m scripts.smoke_tui_read_only`).

### Что UX может делать (разрешено)

#### Допустимые read-only UX-наблюдения (HARD, whitelist)

UX / TUI имеет право **ТОЛЬКО отображать** следующие наблюдаемые факты,
полученные **исключительно** из read-only чтения файлов на диске,
без интерпретации, рекомендаций или принятия решений:

1. **Наличие / отсутствие артефактов состояния**:
   - наличие файлов в `state/snapshots/`, `state/approvals/`, `state/merges/`;
   - наличие каталогов и файлов в `outputs/pass_2/`.

2. **Идентификаторы и имена артефактов**:
   - `snapshot_id`, `run_id`, `merge_id` как строки,
   - имена файлов и каталогов,
   - значения sha256 / hashprefix **как отображаемые строки**, без оценки.

3. **Наблюдаемая классификация FSM-состояния**:
   - отображение текущего FSM-состояния
     **строго как ярлыка**, выведенного из фактов на диске,
     без трактовки допустимости или недопустимости дальнейших действий.

4. **Информационное отображение контрактных правил README.md**:
   - Contract-allowed commands (**informational only**);
   - Contract-forbidden commands (**informational only**);
   - причины запрещённости формулируются
     **исключительно** в терминах контрактных предикатов
     (например: `approval missing`, `merge-state exists`,
     `fingerprint mismatch`).

Ни один из пунктов выше **НЕ является**:
- разрешением,
- рекомендацией,
- сигналом к действию,
- гарантией успешного выполнения команды.

- читать **только факты на диске** (включая `state/` и `outputs/`) в read-only режиме;
- отображать эти факты пользователю (списки файлов, идентификаторы, наличие/отсутствие артефактов);
- отображать **информационную проекцию** контрактных правил README.md в виде:
  - **Contract-allowed commands (informational only) / Contract-forbidden commands (informational only)**,
  - с причиной **исключительно** в терминах контрактных предикатов (например: "approval missing", "merge-state exists", "fingerprint mismatch"),
  - без обещаний успеха и без "разрешений".

### Что UX не имеет права делать (запрещено, HARD)

#### Запрещённая интерпретация (UX interpretation, HARD)

UX / TUI **СТРОГО ЗАПРЕЩЕНО**:

- трактовать наблюдаемые факты как:
  - «можно продолжать»,
  - «всё корректно»,
  - «следующий шаг»,
  - «pipeline готов / сломан»;
- выводить итоговые вердикты корректности состояния;
- агрегировать факты в логические сценарии или workflow;
- упорядочивать действия пользователя
  («сначала сделай X, потом Y»);
- подменять enforcing CLI
  в определении допустимости команд или состояний.

Любая логика вида
«если наблюдается A и B, значит разрешено C»
считается **интерпретацией**
и является нарушением non-authoritative статуса UX.

- принимать lifecycle-решения (включая "можно/нельзя", "всё ок", "следующий шаг");
- валидировать инварианты или заменять PRE-FLIGHT / post-check;
- вычислять "истину" о корректности результатов (valid/invalid) вместо enforcing CLI;
- писать в `state/`, `outputs/`, `input/`, `prompts/`, `.github/` или корень репозитория;
- запускать enforcing CLI как subprocess (UX не является драйвером pipeline);
- предлагать или навязывать пользователю порядок действий (“сначала X, потом Y”) в виде workflow;
- хранить собственное состояние lifecycle или подменять source of truth;
- иметь enforcing-зависимости в смысле раздела
  «Определение enforcing-зависимости (HARD, КАНОНИЧЕСКОЕ)»
  (включая импорт `TYPE: enforcement` модулей, dynamic import, re-export и "только константы/утилиты").

### Правило непротиворечивости (HARD)

#### UX invariants — неизменяемые свойства UX (HARD)

Следующие утверждения должны быть истинны для **любого** UX/TUI/GUI/Web UI в проекте:

1. UX является **non-authoritative**: UX никогда не является источником истины.
2. UX является **read-only наблюдателем**: UX отображает факты и контракт, но не исполняет pipeline.
3. UX **не формирует решения**: UX не выдаёт PASS/FAIL/BLOCKER-вердикты и не подменяет enforcing CLI.
4. UX **не является драйвером workflow**: UX не ведёт пользователя по шагам и не формирует «план выполнения».
5. UX **не агрегирует истину**: UX не строит «сводный статус проекта» или «итоговую оценку состояния».
6. Любая допустимая классификация (FSM-label, Allowed/Forbidden) является **только отображением** и не является разрешением/запретом.

Нарушение любого UX-invariant трактуется как:
- нарушение non-authoritative статуса UX,
- нарушение governance-инварианта,
- основание для `[BLOCKER] GOVERNANCE_VIOLATION`.

#### UX drift — контрактное определение (HARD)

**UX drift** — это любое поведение UX / TUI, при котором:

- read-only наблюдение превращается в:
  - интерпретацию,
  - рекомендацию,
  - оценку,
  - решение;
- UX начинает выглядеть как
  «драйвер workflow» или «планировщик шагов»;
- пользователь может воспринять вывод UX
  как разрешение или запрет действий.

UX drift считается **архитектурным нарушением**,

#### Запрет влияния UX на Human Decision Surface (HARD)

UX / TUI **СТРОГО ЗАПРЕЩЕНО**:

- влиять на решения человека,
- формировать ощущение «правильного выбора»,
- подталкивать к APPROVE или EXECUTE,
- визуально выделять «рекомендуемые» действия,
- представлять Human Decision Surface как UX-workflow.

Любое влияние UX на решения человека
считается **UX drift + governance violation**
и является основанием для `[BLOCKER] GOVERNANCE_VIOLATION`.

даже если:
- запись в `state/` отсутствует;
- smoke-тест read-only проходит;
- UX физически ничего не исполняет.

Любой зафиксированный UX drift трактуется как:
- нарушение non-authoritative статуса UX,
- нарушение governance-инварианта,
- основание для `[BLOCKER] GOVERNANCE_VIOLATION`.

#### UX anti-patterns (BLOCKER) — запрещённые формы UX, независимо от реализации

Следующие паттерны UX считаются **UX drift по определению**
и являются нарушением non-authoritative статуса,
даже если UX read-only и ничего не исполняет:

1. **Progress / workflow UI**:
   - прогресс-бары по lifecycle,
   - чеклисты шагов («1/6 выполнено», «осталось сделать…»),
   - «мастер»/wizard, который ведёт по состояниям.

2. **Project-level verdict UI**:
   - «Project status: OK/FAIL/BLOCKED»,
   - «Pipeline healthy / broken»,
   - «готово к следующему шагу».

3. **Рекомендации и “next step”**:
   - «сделай дальше X»,
   - «следующий шаг: approve/execute/merge»,
   - подсказки, которые выглядят как руководство к действию.

4. **Оценочные индикаторы**:
   - зелёное/красное «хорошо/плохо» как итоговая оценка состояния,
   - скоринг, рейтинги, «quality score».

5. **Автоматическая агрегация фактов в сценарии**:
   - любая логика вида «если A и B, значит C»,
   - формирование причинно-следственных выводов вместо контрактных предикатов.

Любой UX anti-pattern трактуется как:
- UX drift,
- нарушение governance-инварианта,
- основание для `[BLOCKER] GOVERNANCE_VIOLATION`,
даже если:
...
- Если UX показывает "Allowed/Forbidden" или FSM-классификацию, это **только отображение** правил README.md.
- Любой UX обязан оставаться **non-authoritative**: enforcing CLI остаётся единственным механизмом принятия решения и выдачи PASS/FAIL/BLOCKER.

---

## TUI (Textual UI) — read-only наблюдатель (частный случай UX)

### Verification (HARD)

**Minimal TUI v1** считается **verified read-only UX-проекцией** при одновременном выполнении всех условий:

1. Entrypoint `python scripts/tui.py` присутствует в governance-таблице entrypoints
   и классифицирован как `helper / read-only`.
2. `scripts.audit_entrypoints.py` не обнаруживает enforcing-зависимостей
   для `tui.py`.
3. Smoke-тест  
   `python -m scripts.smoke_tui_read_only`  
   завершается **PASS**, что доказывает отсутствие write-side-effects
   (отсутствие изменений дерева репозитория до/после запуска TUI).

PASS smoke-теста `smoke_tui_read_only` является **обязательным verification-критерием**
read-only статуса **Minimal TUI v1**.

Отсутствие или FAIL любого из пунктов выше трактуется как
`[BLOCKER] GOVERNANCE_VIOLATION`.
...
**Роль (строго):**  
Textual TUI — это read-only UI, который показывает:
1) факты на диске (state/outputs),
2) информационную проекцию "что разрешено/запрещено контрактом" для enforcing CLI,
не выполняя действий и не подменяя enforcement.

TUI **не запускает pipeline**, **не выполняет команды**, **не принимает решений**
и **не предлагает "следующий шаг"**.

TUI может показывать:
- **сырые наблюдаемые факты** на диске (state/, outputs/)
- но **не имеет права**:
  - формировать сценарии действий,
  - выстраивать "логический порядок шагов",
  - представлять lifecycle как пользовательский workflow
- **наблюдаемую классификацию FSM-состояния** как **ярлык состояния по фактам на диске**,
  который:
  - не является разрешением или запретом,
  - не означает допустимость следующего шага,
  - не подменяет enforcing CLI,
  - используется исключительно как read-only label.
  а не как разрешение, рекомендацию или сигнал допустимости действий
  (это не "решение" и не "оценка", а статическая классификация)
- **Contract-allowed commands (informational only) / Contract-forbidden commands (informational only)** с причиной как ссылкой на контрактное условие  
  (строго информационно, не разрешение и не рекомендация; enforcing CLI всё равно может вернуть FAIL/BLOCKER)
...
**Что TUI НЕ делает (запрещено, HARD):**
- не является enforcing-гейтом: не "разрешает/запрещает выполнение", а только отображает read-only проекцию по контракту README.md
- не предлагает «следующий шаг»
- не валидирует snapshot или deliverables
- не запускает CLI-команды
- не пишет файлы
- не хранит собственного состояния lifecycle (никаких "истин" помимо фактов на диске)
...
- **не импортирует enforcing-модули** (прямо или транзитивно) **ни в какой форме**, включая:
  ...
  **Каноническое правило (HARD):**
  TUI запрещено импортировать **любой** enforcing-код, определённый в разделе
  «Определение enforcing-зависимости (HARD, КАНОНИЧЕСКОЕ)».
  То есть запрещён импорт любого модуля/файла, который по README.md классифицирован как:
  - `TYPE: enforcement`, или
  - `Класс: enforcing` в governance-таблице entrypoints.

  Ниже приведены **примеры** enforcing-модулей (не whitelist и не исчерпывающий список),
  включая любые обращения к ним по строковым именам:
  ...
  Любая зависимость TUI от enforcing-кода (включая "просто константы/утилиты") запрещена.
...
- TUI не участвует в lifecycle
- TUI не имеет контрактной силы
...
Это напрямую относится к UI (GUI/Web UI): новый UI, не перечисленный в «Структура репозитория»
и не добавленный в governance-таблицу entrypoints, считается **non-canonical** и запрещён к использованию.
...
### Нарушения Human Decision Surface (BLOCKER, HARD)

Важно: **UX / TUI / GUI не являются частью Human Decision Surface**.

Любое человеческое решение,
принятое на основе:
- UX-подсказок,
- визуальных индикаторов,
- списков “разрешено / запрещено”,
- FSM-меток,
- “следующих шагов”,

**НЕ СЧИТАЕТСЯ человеческим решением по контракту**.
...
использование его вывода для принятия решения
является **архитектурным нарушением**.

Human Decision Surface существует **ТОЛЬКО**
через явные файловые артефакты,
перечисленные в этом разделе,
и **НИКАК ИНАЧЕ**.

Следующие действия человека **СТРОГО ЗАПРЕЩЕНЫ** и считаются
нарушением архитектурного контракта,
даже если pipeline формально отработал:

- ручное редактирование файлов в `state/` после их создания кодом;
- ручное редактирование `outputs/` с целью «починить результат»;
- принятие решений на основе UX/TUI/GUI-подсказок;
- запуск lifecycle-команд «по наитию», а не по контракту README.md;
- игнорирование BLOCKER с аргументом «человек так решил»;
- трактовка UX-отображений как разрешений или рекомендаций.

Нарушение Human Decision Surface трактуется как:
- нарушение governance-инварианта,
- `[BLOCKER] GOVERNANCE_VIOLATION`,
- архитектурно недействимый результат,
независимо от exit codes CLI.
